...
const MultiValueCheckbox: React.FC<Props> = ({ className, checked = [], onChange, disabled }) => {
  const t = useT();
  const handleInput = useCallback(
    (e: CheckboxChangeEvent, id: number) => {
      onChange?.(checked?.map((valueItem, index) => (index === id ? e.target.checked : valueItem)));
    },
    [onChange, checked],
  );
      ...
      {Array.isArray(checked) &&
        checked?.map((valueItem, key) => (
          <FieldWrapper key={key}>
            ...
            <Checkbox
              onChange={(e: CheckboxChangeEvent) => handleInput(e, key)}
              checked={valueItem}
            />
            <FlexSpace />
            {!disabled && (
...
export default MultiValueCheckbox;
