...
describe("PreviewModal", () => {
  test("renders image with correct src when visible", () => {
    render(<PreviewModal url="https://example.com/image.png" visible={true} onCancel={vi.fn()} />);
    const img = screen.getByAltText("asset-preview");
    expect(img).toHaveAttribute("src", "https://example.com/image.png");
  });

  test("does not show image when not visible", () => {
    render(<PreviewModal url="https://example.com/image.png" visible={false} onCancel={vi.fn()} />);
