...
export class SchemaPage extends BasePage {
  ...
  async expectModelInPosition(position: number, modelName: string): Promise<void> {
    await expect(this.getByRole('main').getByRole('menuitem').nth(position)).toContainText(modelName);
  }
  ...
  async expectFieldInPosition(position: number, fieldName: string, fieldKey: string): Promise<void> {
    await expect(this.getByLabel('Fields').locator('.draggable-item').nth(position))
      .toContainText(`${fieldName}#${fieldKey}`);
  }
  ...
  async dragGroup(fromPosition: number, toPosition: number): Promise<void> {
    const sourceGroup = this.getByRole('main').getByRole('menu').last().getByRole('menuitem').nth(fromPosition);
    const targetGroup = this.getByRole('main').getByRole('menu').last().getByRole('menuitem').nth(toPosition);
    await sourceGroup.dragTo(targetGroup);
    await this.closeNotification();
  }

  async expectGroupInPosition(position: number, groupName: string): Promise<void> {
    await expect(this.getByRole('main').getByRole('menu').last().getByRole('menuitem').nth(position))
