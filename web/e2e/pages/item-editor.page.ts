...
export class ItemEditorPage extends BasePage {
  ...
  async fillTextField(fieldName: string, value: string): Promise<void> {
    const field = this.getByLabel(fieldName)
      .or(this.locator(`input[placeholder="${fieldName}"]`))
      .or(this.locator(`textarea[placeholder="${fieldName}"]`));
    await field.click();
  ...
  async fillNumberField(fieldName: string, value: string): Promise<void> {
    const field = this.getByLabel(fieldName).or(this.locator(`input[placeholder="${fieldName}"]`));
    await field.fill(value);
  }
  ...
  async fillGroupField(groupName: string, fieldName: string, value: string): Promise<void> {
    ...
    const field = groupSection
      .locator(`input[placeholder="${fieldName}"]`)
      .or(groupSection.getByLabel(fieldName));
    await field.fill(value);
  }
  ...
  async expectSaveSuccess(): Promise<void> {
    await expect(
      this.locator(".ant-notification-notice").filter({ hasText: "successfully" }),
    ).toBeVisible();
  }
  ...
  async expectBooleanState(fieldName: string, checked: boolean): Promise<void> {
    ...
    await expect(this.getByLabel(fieldName).or(this.getByRole("switch"))).toHaveAttribute(
      "aria-checked",
      expectedState,
    );
  }
