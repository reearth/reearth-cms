...
test(
  ...
  async ({ fieldEditorPage, contentPage, schemaPage }) => {
    ...
    await test.step("Verify field appears in content page with default value", async () => {
      ...
      await expect(contentPage.fieldInput(fieldName)).toHaveValue(defaultValue);
      await contentPage.saveButton.click();
      await contentPage.closeNotification();
      await contentPage.backButtonLabel.click();
      await expect(contentPage.tableBodyRows).toContainText(defaultValue);
    });
    ...
    await test.step("Verify updated field in content with multiple values", async () => {
      ...
      await expect(contentPage.cellByText(defaultValue)).toBeVisible();
      await contentPage.newItemButton.click();
      ...
      await expect(contentPage.firstTextbox).toHaveValue(testValue2);
