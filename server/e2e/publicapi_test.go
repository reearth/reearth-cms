...
var (
	...
	publicAPIField1lKey   = "test-field-1"
	publicAPIField2lKey   = "asset"
)
...
func publicAPISeeder(ctx context.Context, r *repo.Container) error {
	...
	a := asset.New().ID(publicAPIAsset1ID).Project(p1.ID()).CreatedByUser(uid).Size(1).Thread(id.NewThreadID()).File(
		asset.NewFile().Name("aaa.txt").Path("aaa.txt").Build(),
	).UUID(publicAPIAssetUUID).MustBuild()

	s := schema.New().NewID().Project(p1.ID()).Workspace(p1.Workspace()).Fields(schema.FieldList{
		schema.NewField(schema.NewText(nil).TypeProperty()).NewID().Key(key.New(publicAPIField1lKey)).MustBuild(),
		schema.NewField(schema.NewAsset().TypeProperty()).NewID().Key(key.New(publicAPIField2lKey)).MustBuild(),
	}).MustBuild()
