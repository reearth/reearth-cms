...
func Test_AreItemsReferenced(t *testing.T) {
	...
	if1 := NewField(fid1, value.TypeReference.Value(iid2.String()).AsMultiple())
	if2 := NewField(fid2, value.TypeReference.Value(iid1.String()).AsMultiple())
	if3 := NewField(fid3, value.TypeReference.Value(id.NewItemID().String()).AsMultiple())
