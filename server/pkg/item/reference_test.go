...
func Test_AreItemsReferenced(t *testing.T) {
	...
	i1 := New().ID(iid1).Schema(sid1).Model(mid1).Project(prj.ID()).Fields([]*Field{if1}).Thread(id.NewThreadID().Ref()).Anonymous(true).MustBuild()
	i2 := New().ID(iid2).Schema(sid2).Model(mid2).Project(prj.ID()).Fields([]*Field{if2}).Thread(id.NewThreadID().Ref()).Anonymous(true).MustBuild()
	i3 := New().ID(iid3).Schema(sid3).Model(mid3).Project(prj.ID()).Fields([]*Field{if3}).Thread(id.NewThreadID().Ref()).Anonymous(true).MustBuild()
