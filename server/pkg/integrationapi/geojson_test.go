...
func TestGetGeometryField(t *testing.T) {
	iid := id.NewItemID()
	sid := id.NewSchemaID()
	mid := id.NewModelID()
	uid := accountdomain.NewUserID()
	...
	i2 := item.New().
		...
		Schema(sid).
		Project(pid).
		Fields([]*item.Field{fi1, fi2, fi4, fi5}).
		Model(mid).
		Thread(tid).
		User(uid).
		Integration(nid).
