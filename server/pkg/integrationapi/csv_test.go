...
func TestToSingleString(t *testing.T) {
	...
	s10, ok10 := toSingleString(if10)
	assert.Equal(t, v10.Format(time.RFC3339), s10)
	assert.True(t, ok10)
  
	var if11 *item.Field
	s11, ok11 := toSingleString(if11)
	assert.Empty(t, s11)
	assert.False(t, ok11)
}