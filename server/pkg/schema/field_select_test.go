...
func TestFieldSelectFrom(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "success values",
			args:    args{values: lo.ToPtr([]string{"v1", "v2"})},
			want:    &FieldSelect{values: lo.ToPtr([]string{"v1", "v2"})},
			wantErr: nil,
		},
		{
			name:    "success values and default value",
			args:    args{values: lo.ToPtr([]string{"v1", "v2"}), defaultValue: lo.ToPtr(0)},
			want:    &FieldSelect{values: lo.ToPtr([]string{"v1", "v2"}), defaultValue: lo.ToPtr(0)},
			wantErr: nil,
		},
		{
			name:    "fail no values",
		...
		},
		{
			...
			args:    args{values: lo.ToPtr([]string{"v1", "v2"}), defaultValue: lo.ToPtr(3)},
			want:    nil,
			wantErr: ErrFieldDefaultValue,
		},
		{
			name:    "fail values and default value",
			args:    args{values: lo.ToPtr([]string{"v1", "v2"}), defaultValue: lo.ToPtr(-1)},
			want:    nil,
			wantErr: ErrFieldDefaultValue,
		},
	}
...
func TestFieldSelect_TypeProperty(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "success values",
			f:    &FieldSelect{values: lo.ToPtr([]string{"V1", "v2"})},
			want: &TypeProperty{selectt: &FieldSelect{values: lo.ToPtr([]string{"V1", "v2"})}},
		},
		{
