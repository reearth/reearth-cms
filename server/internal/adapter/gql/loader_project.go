...
func (c *ProjectLoader) FindByWorkspace(ctx context.Context, workspaceId gqlmodel.ID, first *int, last *int, before *usecase.Cursor, after *usecase.Cursor) (*gqlmodel.ProjectConnection, error) {
	wid, err := gqlmodel.ToID[id.Workspace](workspaceId)
	if err != nil {
		return nil, err
	}

	res, pi, err := c.usecase.FindByWorkspace(ctx, wid, usecase.NewPagination(first, last, before, after), getOperator(ctx))
	if err != nil {
		return nil, err
	}
