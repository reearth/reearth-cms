...
// TestItemRepo runs the interface contract tests for repo.Item. newRepo must
// return a fresh, empty, unfiltered repository for each call.
func TestItemRepo(t *testing.T, newRepo func(t *testing.T) repo.Item) {
	ctx := context.Background()

	// items compared by deep equality are built with a millisecond-truncated
	// UTC timestamp so that they survive a persistence round-trip unchanged
	...
	t.Run("FindByID", func(t *testing.T) {
		...
		i1 := item.New().NewID().Fields(fs).Schema(id.NewSchemaID()).Model(id.NewModelID()).Thread(id.NewThreadID().Ref()).Project(id.NewProjectID()).Timestamp(ts).MustBuild()
	...
	t.Run("FindByIDs", func(t *testing.T) {
		...
		sfid := schema.NewFieldID()
		fs := []*item.Field{item.NewField(sfid, value.TypeBool.Value(true).AsMultiple(), nil)}
		i1 := item.New().NewID().Fields(fs).Schema(sid).Model(id.NewModelID()).Project(pid).Thread(id.NewThreadID().Ref()).Timestamp(ts).MustBuild()
		i2 := item.New().NewID().Fields(fs).Schema(sid).Model(id.NewModelID()).Project(pid).Thread(id.NewThreadID().Ref()).Timestamp(ts).MustBuild()
	...
	t.Run("FindAllVersionsByID", func(t *testing.T) {
	...
	t.Run("FindAllVersionsByIDs", func(t *testing.T) {
		now1 := time.Date(2022, time.April, 1, 0, 0, 0, 0, time.UTC)
		now2 := time.Date(2022, time.April, 2, 0, 0, 0, 0, time.UTC)
		ts1 := now1.Truncate(time.Millisecond)
		ts2 := ts1.Add(time.Second)
	...
	t.Run("FindBySchema", func(t *testing.T) {
		sid := id.NewSchemaID()
		pid := id.NewProjectID()
		sfid := schema.NewFieldID()
		fs := []*item.Field{item.NewField(sfid, value.TypeBool.Value(true).AsMultiple(), nil)}
		i1 := item.New().NewID().Fields(fs).Schema(sid).Model(id.NewModelID()).Thread(id.NewThreadID().Ref()).Project(pid).Timestamp(ts).MustBuild()
		i2 := item.New().NewID().Fields(fs).Schema(sid).Model(id.NewModelID()).Thread(id.NewThreadID().Ref()).Project(pid).Timestamp(ts).MustBuild()
		i3 := item.New().NewID().Fields(fs).Schema(sid).Model(id.NewModelID()).Thread(id.NewThreadID().Ref()).Project(id.NewProjectID()).Timestamp(ts).MustBuild()
		i4 := item.New().NewID().Fields(fs).Schema(id.NewSchemaID()).Model(id.NewModelID()).Thread(id.NewThreadID().Ref()).Project(pid).Timestamp(ts).MustBuild()
	...
	t.Run("Save", func(t *testing.T) {
		i1 := item.New().NewID().Schema(id.NewSchemaID()).Model(id.NewModelID()).Thread(id.NewThreadID().Ref()).Project(id.NewProjectID()).Timestamp(ts).MustBuild()
		i2 := item.New().NewID().Schema(id.NewSchemaID()).Model(id.NewModelID()).Thread(id.NewThreadID().Ref()).Project(id.NewProjectID()).Timestamp(ts).MustBuild()
	...
	t.Run("Remove", func(t *testing.T) {
		...
		// the result of the call is backend-defined (memory reports
		// ErrOperationDenied, mongo cannot match the item and is a no-op)
	...
	t.Run("BatchRemove", func(t *testing.T) {
		pid := id.NewProjectID()
		i1 := item.New().NewID().Schema(id.NewSchemaID()).Model(id.NewModelID()).Project(pid).Thread(id.NewThreadID().Ref()).MustBuild()
		i2 := item.New().NewID().Schema(id.NewSchemaID()).Model(id.NewModelID()).Project(pid).Thread(id.NewThreadID().Ref()).MustBuild()
		...
		t.Run("permission denied", func(t *testing.T) {
			...
			// removing an item outside the writable projects must not remove
			// it; the result of the call is backend-defined (memory reports
			// ErrOperationDenied, mongo cannot match the item and is a no-op)
	...
	t.Run("FindByModelAndValue", func(t *testing.T) {
		sid := id.NewSchemaID()
		sf1 := id.NewFieldID()
		sf2 := id.NewFieldID()
	...
	t.Run("FindByAssets", func(t *testing.T) {
		sid := id.NewSchemaID()
		sf1 := id.NewFieldID()
		sf2 := id.NewFieldID()
		pid := id.NewProjectID()
