...
func (i *Project) checkPermission(ctx context.Context, operator *usecase.Operator, workspaceID *workspace.ID, caller, action string) error {
...
func (i *Project) Fetch(ctx context.Context, ids []id.ProjectID, operator *usecase.Operator) (project.List, error) {
	...
	if len(projects) > 0 {
		ws := projects[0].Workspace()
		wid = &ws
	}
	if err := i.checkPermission(ctx, operator, wid, "project.Fetch", rbac.ActionList); err != nil {
		return nil, err
	}
	return projects, nil
}
...
func (i *Project) FindByIDOrAlias(ctx context.Context, wsIdOrAlias accountdomain.WorkspaceIDOrAlias, idOrAlias project.IDOrAlias, operator *usecase.Operator) (*project.Project, error) {
	...
	wid := w.ID()
	if err := i.checkPermission(ctx, operator, &wid, "project.FindByIDOrAlias", rbac.ActionRead); err != nil {
		return nil, err
	}
...
func (i *Project) Update(ctx context.Context, param interfaces.UpdateProjectParam, op *usecase.Operator) (_ *project.Project, err error) {
	...
	ws := p.Workspace()
	if err := i.checkPermission(ctx, op, &ws, "project.Update", rbac.ActionUpdate); err != nil {
		return nil, err
	}
...
func (i *Project) Delete(ctx context.Context, projectID id.ProjectID, op *usecase.Operator) (err error) {
	...
	projWS := proj.Workspace()
	if err := i.checkPermission(ctx, op, &projWS, "project.Delete", rbac.ActionDelete); err != nil {
		return err
	}
...
func (i *Project) CreateAPIKey(ctx context.Context, param interfaces.CreateAPITokenParam, op *usecase.Operator) (*project.Project, *project.APIKeyID, error) {
	...
	pws := p.Workspace()
	if err := i.checkPermission(ctx, op, &pws, "project.CreateAPIKey", rbac.ActionManageAPIKeys); err != nil {
		return nil, nil, err
	}
...
func (i *Project) UpdateAPIKey(ctx context.Context, param interfaces.UpdateAPITokenParam, op *usecase.Operator) (*project.Project, error) {
	...
	pws2 := p.Workspace()
	if err := i.checkPermission(ctx, op, &pws2, "project.UpdateAPIKey", rbac.ActionManageAPIKeys); err != nil {
		return nil, err
	}
...
func (i *Project) DeleteAPIKey(ctx context.Context, pId id.ProjectID, kId id.APIKeyID, op *usecase.Operator) (*project.Project, error) {
	...
	pws3 := p.Workspace()
	if err := i.checkPermission(ctx, op, &pws3, "project.DeleteAPIKey", rbac.ActionManageAPIKeys); err != nil {
		return nil, err
	}
...
func (i *Project) RegenerateAPIKeyKey(ctx context.Context, param interfaces.RegenerateKeyParam, op *usecase.Operator) (*project.Project, error) {
	...
	pws4 := p.Workspace()
	if err := i.checkPermission(ctx, op, &pws4, "project.RegenerateAPIKeyKey", rbac.ActionManageAPIKeys); err != nil {
		return nil, err
	}
...
func (i *Project) StarProject(ctx context.Context, wsIdOrAlias accountdomain.WorkspaceIDOrAlias, idOrAlias project.IDOrAlias, op *usecase.Operator) (_ *project.Project, err error) {
	...
	wID := w.ID()
	if err := i.checkPermission(ctx, op, &wID, "project.StarProject", rbac.ActionUpdate); err != nil {
		return nil, err
	}
