...
func (i *Asset) BatchDelete(ctx context.Context, assetIDs id.AssetIDList, operator *usecase.Operator) (result []id.AssetID, err error) {
	...
	return Run1(
		...
		func(ctx context.Context) (id.AssetIDList, error) {
			...
			mapFileNameByUUID := lo.FilterSliceToMap(assets, func(a *asset.Asset) (string, string, bool) {
				if a == nil || a.UUID() == "" || a.FileName() == "" {
					return "", "", false
				}
				return a.UUID(), a.FileName(), true
			})

			// deletes assets' files in
			err = i.gateways.File.DeleteAssets(ctx, mapFileNameByUUID)
			if err != nil {
				return assetIDs, err
			}
