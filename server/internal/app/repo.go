...
func initReposAndGateways(ctx context.Context, conf *Config, debug bool) (*repo.Container, *gateway.Container) {
	...
	if conf.GCS.BucketName != "" {
		...
		fileRepo, err = aws.NewFile(conf.S3.BucketName, conf.S3.AccessKeyID, conf.S3.SecretAccessKey, conf.S3.Region, conf.AssetBaseURL, conf.S3.PublicationCacheControl)
		if err != nil {
