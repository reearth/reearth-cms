...
func TestItem_FindAllVersionsByID(t *testing.T) {
	...
	i1 := item.New().ID(iid).Fields(fs).Schema(id.NewSchemaID()).Model(id.NewModelID()).Thread(id.NewThreadID()).Project(pid).MustBuild()
	restore()
	now2 := now1.Add(time.Second).UTC()
	restore = util.MockNow(now2)
	i2 := item.New().ID(iid).Fields(fs).Schema(i1.Schema()).Model(id.NewModelID()).Thread(id.NewThreadID()).Project(i1.Project()).MustBuild()
	restore()
...
func TestItem_Remove(t *testing.T) {
	...
	sfid := schema.NewFieldID()
	fs := []*item.Field{item.NewField(sfid, schema.TypeBool, true)}
	i1 := item.New().ID(id1).Fields(fs).Schema(sid).Model(id.NewModelID()).Thread(id.NewThreadID()).Project(pid).Project(pid).MustBuild()
	init := mongotest.Connect(t)
	client := mongox.NewClientWithDatabase(init(t))
