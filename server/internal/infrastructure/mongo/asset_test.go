...
func TestAssetRepo_FindByID(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "Not found",
			seeds: []*asset.Asset{
				asset.New().ID(id1).Project(pid1).CreatedBy(uid1).Size(1000).File(f).Thread(id.NewThreadID()).MustBuild(),
			},
			arg:     id.NewAssetID(),
			want:    nil,
			wantErr: rerror.ErrNotFound,
...
func TestAssetRepo_Delete(t *testing.T) {
	...
	id1 := id.NewAssetID()
	f := asset.File{}
	a1 := asset.New().ID(id1).Project(pid1).CreatedBy(uid1).Size(1000).File(&f).Thread(id.NewThreadID()).MustBuild()
	tests := []struct {
		name  string
		seeds []*asset.Asset
		arg   id.AssetID
		...
		},
		{
			name: "Not found",
			seeds: []*asset.Asset{
				asset.New().NewID().Project(pid1).CreatedBy(uid1).Size(1000).File(&f).Thread(id.NewThreadID()).MustBuild(),
			},
			arg:     id.NewAssetID(),
			wantErr: rerror.ErrNotFound,
		},
