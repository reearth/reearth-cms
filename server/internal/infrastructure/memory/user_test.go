...
func TestUser_FindByAuth0Sub(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			repo: &User{
				data: map[id.UserID]*user.User{u.ID(): u},
			},
			auth0sub: "xxx",
			want:     u,
			wantErr:  false,
	...
	for _, tc := range tests {
		t.Run(tc.name, func(tt *testing.T) {
			tt.Parallel()
			got, err := tc.repo.FindByAuth0Sub(ctx, tc.auth0sub)
			if tc.wantErr {
				assert.Error(tt, err)
			} else {
...
func TestUser_FindByEmail(t *testing.T) {
	...
	_, err = r.FindByEmail(ctx, "abc@bb.cc")
...
func TestUser_FindByName(t *testing.T) {
	...
	out, err := r.FindByName(ctx, "hoge")
	assert.NoError(t, err)
	assert.Equal(t, u, out)

	_, err = r.FindByName(ctx, "xxx")
	assert.Same(t, rerror.ErrNotFound, err)
}
...
func TestUser_Remove(t *testing.T) {
	ctx := context.Background()
	u := user.New().NewID().Name("hoge").Email("aa@bb.cc").MustBuild()
	u2 := user.New().NewID().Name("hoge").Email("abc@bb.cc").MustBuild()
	r := &User{
		data: map[id.UserID]*user.User{
			u.ID():  u,
			u2.ID(): u2,
	...
	expected := map[id.UserID]*user.User{
		u.ID(): u,
	}
